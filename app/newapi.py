...
class GetNotDoneOrder(BaseHandler):
    ...
    def getdata(self):
        ...
        if res is not None:
            data = {'order_id':res.order_id,'timestramp_str':res.order_timestrampstr,'order_paySign':res.order_paySign,'order_wx_prepay_id':res.order_wx_prepay_id,'order_nonceStr':res.order_nonceStr,'order_code':res.order_code,'packagename': res.package_name,'packagemoney':res.package_money,'ep_name':res.ep_name, 'order_money':res.order_money,'order_state':res.order_state,'order_type':res.order_type}
        else:
            data = 0
        self.DbRead.commit()
        self.DbRead.close()
