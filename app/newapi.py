...
class GetNotDoneOrder(BaseHandler):
    ...
    @run_on_executor
    def getdata(self):
        res = self.DbRead.query(self.Order.order_id, self.Order.order_timestrampstr, self.Order.order_paySign, self.Order.order_wx_prepay_id, self.Order.order_nonceStr, self.Package.package_money, self.Order.order_state, self.Order.order_type, self.Exam_place.ep_name, self.Package.package_name, self.Order.order_code, self.Order.order_money)\
        ...
        if res is not None:
            data = {'order_id': res.order_id, 'timestramp_str': res.order_timestrampstr, 'order_paySign': res.order_paySign, 'order_wx_prepay_id': res.order_wx_prepay_id, 'order_nonceStr': res.order_nonceStr,
                    'order_code': res.order_code, 'packagename': res.package_name, 'packagemoney': res.package_money, 'ep_name': res.ep_name, 'order_money': res.order_money, 'order_state': res.order_state, 'order_type': res.order_type}
        else:
            data = 0
        self.DbRead.commit()
        self.DbRead.close()
