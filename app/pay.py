...
class SetOrder(BaseHandler):
    ...
    def XmlData(self):
        ...
        formatstr = 'appid=%s&attach=%s&mch_id=%s&body=%s&nonce_str=%s&notify_url=%s&openid=%s&out_trade_no=%s&spbill_create_ip=%s&total_fee=%s&trade_type=%s&key=%s' % (appidvalue, attachvalue,mch_idvalue,str(bodyvalue),nonce_strvalue,notify_urlvalue,openidvalue,out_trade_novalue, spbill_create_ipvalue, total_feevalue,trade_typevalue, key)
        print formatstr
